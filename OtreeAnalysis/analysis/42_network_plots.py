...
Individual pages also saved to ../fig/networks/individual/<key>.pdf

Output:
  ../fig/networks/all_networks.pdf
  ../fig/networks/individual/<key>.svg  (one per participant)
"""
...
for i, key in enumerate(keys):
    fig = make_page(key, i + 1, n)
    fname = f"{i + 1:02d}_{key}.svg"
    fig.savefig(str(INDIV_DIR / fname), bbox_inches="tight")
    plt.close(fig)

print("Saved:", OUTDIR / "all_networks.pdf")
print("Saved individual SVGs to:", INDIV_DIR)
