'''
VMP 2026-01-14:
This document essentially has the same prompts as what is used in the survey directly (see otreesurvey/otreesurvey_app/llm_utils.py)
For the node extraction we remove the limit of maximum n=10 beliefs extracted.

In particular, the following line is dropped: 
...
'''
All functions used by: 03_llm_nodes.py
'''
...
'''
Run the same node prompt as in the survey, but remove the line that limits it to n=10 beliefs.
''' 
...
Extraction rules: 
1. Extract at least 1 statement about the personal meat-eating habits of the interviewee.
2. Provide the following fields for each statement: 
...
3. If you cannot reliably assign an effect (increase or decrease), do not include the statement.
