...
Writes:
  ../fig/edge_reliability/edge_phi_boxdots.svg      (Figure 4 V2)
  ../fig/edge_reliability/edge_phi_table.tex        (Table S7 V2)
  ../fig/edge_reliability/edge_contingency_table.tex

VMP 2026-02-08: tested and run.
VMP 2026-03-27: renamed from 36_bertopic_edge_reliability.py;
                moved output to /fig/edge_reliability/;
                removed stale docstring entries (participant_level.csv, summary.csv).
"""
...
OUTDIR = Path("../fig/edge_reliability")
...
self_phi  = np.diag(P_edge)
other_phi = P_edge[~np.eye(n_keys, dtype=bool)]
...
mean_se_plot_side(
    ...
    outname=str(OUTDIR / "edge_phi_boxdots.svg"),
    **PLOT_STYLE,
)
print(f"Saved: {OUTDIR / 'edge_phi_boxdots.svg'}")

# ---- Phi table across all 10 candidate runs ----
summary_rows = []
...
with open(OUTDIR / "edge_phi_table.tex", "w", encoding="utf-8") as f:
    f.write(latex)
print(f"Saved: {OUTDIR / 'edge_phi_table.tex'}")


# ---- Contingency table (W1 × W2 edge presence, averaged across participants) ----
a_pp = (A_edge * B_edge).sum(axis=1).astype(float)
b_pp = (A_edge * (1 - B_edge)).sum(axis=1).astype(float)
c_pp = ((1 - A_edge) * B_edge).sum(axis=1).astype(float)
d_pp = ((1 - A_edge) * (1 - B_edge)).sum(axis=1).astype(float)
...
with open(OUTDIR / "edge_contingency_table.tex", "w", encoding="utf-8") as f:
    f.write(contingency_latex)
print(f"Saved: {OUTDIR / 'edge_contingency_table.tex'}")
