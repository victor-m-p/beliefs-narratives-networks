"""
39_collective_network.py

VMP 2026-02-19

Population-level topic network (human canvas edges) with opportunity-based normalization.

Reads:
  ../data/public/bertopic/selection/statement_topics/09__*__statement_topics.csv
  ../data/public/bertopic/selection/overview/09__*__topic_info.csv
  ../data/public/distractors_w*.json

Writes:
  ../fig/collective_network/collective_network.pdf          (Figure 8 V2)
  ../fig/collective_network/collective_network.svg          (Figure 8 V2)
  ../fig/collective_network/all_individuals.pdf
  ../fig/collective_network/individual/<i>_<key>.svg        (one per participant)

Reproducibility:
  spectral ring ordering stabilized (sorted nodes + anchored rotation + optional flip fix);
  spectral ordering is deterministic given node order; SEED is mainly for future layouts.

VMP 2026-03-27: renamed output folder to collective_network; removed REMOVE_OUTLIER toggle
                (always include outliers); removed unsaved preview figure; merged SVG and
                PDF loops.
"""
...
CFG = dict(
    ...
    OUTDIR=Path("../fig/collective_network"),
)
...
fig.savefig(CFG["OUTDIR"] / "collective_network.pdf", bbox_inches="tight")
fig.savefig(CFG["OUTDIR"] / "collective_network.svg", bbox_inches="tight")
